...
async function mentionNotification(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  const pageSize = 100;
...
app.http("mentionNotification", {
  methods: ["POST"],
  authLevel: "anonymous",
  route: "api/mention-notification",
  handler: mentionNotification,
});
