...
import { NotificationBot, TeamsBotInstallation } from "../notification/notification";
...
async function notifyTimerTrigger(myTimer: Timer, context: InvocationContext): Promise<void> {
  ...
  getQuoteBySymbol(alphaVantageClient)("MSFT")
    ...
    .then((quote) =>
      getInstallations(notificationApp).then((targets) =>
        targets.map((target) => sendCard(target)()(template)(quote))
      )
    )
...
const makeValidAdaptiveCardISOString = (iso: string): string =>
  `${iso.split(".")[0]}Z`;

const getQuoteBySymbol =
  (apiClient: AxiosInstance) => (symbol: string) =>
    apiClient.get(`/query?function=GLOBAL_QUOTE&symbol=${symbol}`);

const getResponseData = (res: any): object => res.data;

const extractGlobalQuote = (data: object): GlobalQuote => data["Global Quote"];

const transformGlobalQuote = (quote: GlobalQuote): GlobalQuote => ({
...
const removePercent = (string: string): string => string.replace("%", "");

const addTimestamp =
  (quote: GlobalQuote) =>
  (timestamp: string): GlobalQuote => {
    return { ...quote, timestamp };
  };

const addCompanyName =
  (quote: GlobalQuote) =>
  (name: string): GlobalQuote => {
    return { ...quote, name };
  };
...
const getInstallations = async (
  ...
  do {
    const pagedInstallations = await app.getPagedInstallations(
      pageSize,
      continuationToken
    );
...
const sendCard =
  ...
  (quote: GlobalQuote): Promise<any> =>
    target.sendAdaptiveCard(
      new ACData.Template(template).expand({ $root: quote })
    );
...
app.timer("notifyTimerTrigger", {
  schedule: "*/30 * * * * *",
  handler: notifyTimerTrigger,
});
